...
#define l_rw dk_bit(PORTD,1)
#define l_rw_d dk_bit(DDRD,1)
...
u8 lcd_read_byte(void)
{
	...
	#else
	#if (defined(l_d4_p) && defined(l_d5_p) && defined(l_d6_p) && defined(l_d7_p))
...
void lcd_send_nibble(u8 n)
{
	...
	n>>=1;
	l_d7=(n&0x1);
//	output_bit(l_d4_p, bit_test(n, 0));
//	output_bit(l_d5_p, bit_test(n, 1));
//	output_bit(l_d6_p, bit_test(n, 2));
//	output_bit(l_d7_p, bit_test(n, 3));
	#else
	lcdlat.data = n;
	#endif
...
void l_ky_tu_xy(u8 x,u8 y,char p)
{
	l_toi_xy(x,y);
	lcd_putc(p);
}
//======================================== 
void l_in_xy(u8 x,u8 y,char *p)
{
	l_toi_xy(x,y);
	l_in(p);
}
...
#define l_xoa() l_lenh(0b00000001); lcd_send_byte(0,0x80)
