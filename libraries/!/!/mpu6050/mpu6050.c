...
volatile uint8_t mpu6050_buffer[14];
...
// 		cli();
// 		SoftI2CStart2();
// 		SoftI2CWriteByte2(mpu6050_gi);
// 		SoftI2CWriteByte2(regAddr); // Start at REG 0 - Seconds
// 		SoftI2CStart2();
// 		SoftI2CWriteByte2(0xD1);
// 		dia_chi = SoftI2CReadByte2(0);
// 		SoftI2CStop2();
// 		sei();
		cli();
		//request register
		SoftI2CStart2();
		SoftI2CWriteByte2(mpu6050_gi);
		SoftI2CWriteByte2(regAddr);
		_delay_us(10);
		//read data
		SoftI2CStart2();
		SoftI2CWriteByte2(mpu6050_doc);
		for(i=0; i<length; i++) {
			count++;
			if(i==length-1)
				data[i] = SoftI2CReadByte2(1);
			else
				data[i] = SoftI2CReadByte2(0);
		}
		SoftI2CStop2();
		sei();
	}
...
void mpu6050_writeBytes(uint8_t regAddr, uint8_t length, uint8_t* data) {
	if(length > 0) 
	{
		...
		SoftI2CStart2();
		SoftI2CWriteByte2(mpu6050_gi);
		SoftI2CWriteByte2(regAddr); //reg
		for (uint8_t i = 0; i < length; i++) {
			SoftI2CWriteByte2((uint8_t) data[i]);
		}
		SoftI2CStop2();
		sei();
	}
...
void mpu6050_writeWords(uint8_t regAddr, uint8_t length, uint16_t* data) {
	if(length > 0) 
	{
		...
		uint8_t i = 0;
		//write data
		SoftI2CStart2(MPU6050_ADDR | I2C_WRITE);
		SoftI2CWriteByte2(regAddr); //reg
		for (i = 0; i < length * 2; i++) {
			SoftI2CWriteByte2((uint8_t)(data[i++] >> 8)); // send MSB
			SoftI2CWriteByte2((uint8_t)data[i]);          // send LSB
		}
		SoftI2CStop2();
		sei();
	}
...
uint16_t mpu6050_getFIFOCount() {
	mpu6050_readBytes(MPU6050_RA_FIFO_COUNTH, 2, (uint8_t *)mpu6050_buffer);
...
uint8_t mpu6050_getIntStatus() {
	mpu6050_readByte(MPU6050_RA_INT_STATUS, (uint8_t *)mpu6050_buffer);
...
int8_t mpu6050_getXGyroOffset() {
	mpu6050_readBits(MPU6050_RA_XG_OFFS_TC, MPU6050_TC_OFFSET_BIT, MPU6050_TC_OFFSET_LENGTH, (uint8_t *)mpu6050_buffer);
    return mpu6050_buffer[0];
}
...
int8_t mpu6050_getYGyroOffset() {
	mpu6050_readBits(MPU6050_RA_YG_OFFS_TC, MPU6050_TC_OFFSET_BIT, MPU6050_TC_OFFSET_LENGTH, (uint8_t *)mpu6050_buffer);
    return mpu6050_buffer[0];
}
...
int8_t mpu6050_getZGyroOffset() {
	mpu6050_readBits(MPU6050_RA_ZG_OFFS_TC, MPU6050_TC_OFFSET_BIT, MPU6050_TC_OFFSET_LENGTH, (uint8_t *)mpu6050_buffer);
    return mpu6050_buffer[0];
}
...
uint8_t mpu6050_testConnection() {
	mpu6050_readBits(MPU6050_RA_WHO_AM_I, MPU6050_WHO_AM_I_BIT, MPU6050_WHO_AM_I_LENGTH, (uint8_t *)mpu6050_buffer);
	if(mpu6050_buffer[0] == 0x34)
		return 1;
	else
		return 0;
}
...
void mpu6050_getRawData(int16_t* ax, int16_t* ay, int16_t* az, int16_t* gx, int16_t* gy, int16_t* gz) {
	mpu6050_readBytes(MPU6050_RA_ACCEL_XOUT_H, 14, (uint8_t *)mpu6050_buffer);
...
void mpu6050_updateQuaternion() {
	...
	//get raw data
	while(1) {
		mpu6050_readBit(MPU6050_RA_INT_STATUS, MPU6050_INTERRUPT_DATA_RDY_BIT, (uint8_t *)mpu6050_buffer);
		if(mpu6050_buffer[0])
			break;
		_delay_us(10);
	}
