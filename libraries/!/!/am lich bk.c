...
int8 ngaya,thanga,nama;
const int8 ALdauthangDL[21][12] PROGMEM   = { // ngay âm dâu thang duong lich
...
const int8 thangALdauthangDL[21][12] PROGMEM = { // thang am dau thang duong lich
...
const int8 DLdauthangAL[21][12] PROGMEM = { // ngay duong dau thang am lich
...
const int8 thangALdauthangAL[21][12] PROGMEM = { // thang am co ngay 1 dau tien trong thang 
...
void tinh_am_lich() // tính am lich tu ngay thang nam duong lich
{
   u8 luu;
   u8 ngay_a,thang_a;
   u8 ngay_am_dau_thang_duong,ngay_duong_dau_thang_am;     
   if((nama-10)>20)// cho phep den nam 2030
      return; 
   
   ngay_am_dau_thang_duong = pgm_read_byte(&ALdauthangDL[nama-10][thang-1]); // 19                   
   ngay_duong_dau_thang_am = pgm_read_byte(&DLdauthangAL[nama-10][thang-1]); // 13   
   luu=ngay_duong_dau_thang_am-ngaya; // -10 // 246
   if(luu==0) // khi ngay duong dau thang am trung voi ngay                                           
   {
      ngay_a = 1; // ngay am = 1, ngay dau cua thang am lich
      thang_a = pgm_read_byte(&thangALdauthangAL[nama-10][thanga-1]); // than am là thanh co ngay 1 dau tien trong thang duong lich 
   }
   else                                                                                                                                                           
   if(luu>31) // khi ngay duong dau thang am <= ngay hien tai
   {                                                                                                  
      ngay_a = (ngaya-ngay_duong_dau_thang_am+1);//
      thang_a = pgm_read_byte(&thangALdauthangDL[nama-10][thanga]);
   }  
   else // khi ngay hien tai < ngay duong dau tháng am
   {
      ngay_a = (ngaya+ngay_am_dau_thang_duong-1);//
      thang_a = pgm_read_byte(&thangALdauthangDL[nama-10][thanga-1]);
   }       
   if(thanga-thang_a>100&&thang-thang_a<255)
   {
      nama--;  
   }         
   thanga=thang_a;
