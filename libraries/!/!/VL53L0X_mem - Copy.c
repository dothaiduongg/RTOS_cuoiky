...
{
	 SoftI2CStart();
	 SoftI2CWriteByte(g_i2cAddr | I2C_WRITE); // I2C write address
	 SoftI2CWriteByte(reg); // Start at REG 0 - Seconds
	 SoftI2CWriteByte((value     ) & 0xFF); // REG 2
	 SoftI2CStop();
}
...
void writeReg16Bit(uint8_t reg, uint16_t value)
{
	 ...
	 SoftI2CWriteByte(g_i2cAddr | I2C_WRITE); // I2C write address
	 SoftI2CWriteByte(reg); // Start at REG 0 - Seconds
	 SoftI2CWriteByte((value >> 8) & 0xFF); // REG 1
	 SoftI2CWriteByte((value     ) & 0xFF); // REG 2
	 SoftI2CStop();
}
...
void writeReg32Bit(uint8_t reg, uint32_t value)
{
	 ...
	 SoftI2CWriteByte(g_i2cAddr | I2C_WRITE); // I2C write address
	 SoftI2CWriteByte(reg); // Start at REG 0 - Seconds
	 SoftI2CWriteByte((value >>24) & 0xFF); // REG 0
	 SoftI2CWriteByte((value >>16) & 0xFF); // REG 0
	 SoftI2CWriteByte((value >> 8) & 0xFF); // REG 1
	 SoftI2CWriteByte((value     ) & 0xFF); // REG 2
	 SoftI2CStop();
}
...
uint8_t readReg(uint8_t reg) 
{
	uint8_t value;
  	SoftI2CStart();
  	SoftI2CWriteByte(g_i2cAddr | I2C_WRITE);
  	SoftI2CWriteByte(reg); // Start at REG 0 - Seconds
  	SoftI2CStart();
  	SoftI2CWriteByte(g_i2cAddr | I2C_READ);
  	value = SoftI2CReadByte(0);
  	SoftI2CStop();
	return value;
}
...
{
	uint16_t value;
	value  = (uint16_t)readReg(reg)<<8;
	value |= (uint16_t)readReg(reg+1);
	return value;
}
...
uint32_t readReg32Bit(uint8_t reg) {
  ...
  value |= (uint32_t)readReg(reg+2) << 8;
  value |= readReg(reg+3);
  return value;
}
