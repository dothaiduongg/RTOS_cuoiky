...
void drawHorizontalLine( long x1, long x2, long y0, bool color) {
	if (x1 > x2) {
		swap(x1, x2);
	}
...
void drawVerticalLine(long x0, long y1, long y2, bool color) {
	if (y1 > y2) {
		swap(y1, y2);
	}
	int16_t hieu = y2 - y1;
...
void fillTriangle (uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, uint8_t x3, uint8_t y3, uint8_t color) {
	uint8_t sl, sx1, sx2;
	double m1, m2, m3;
	if (y2 > y3)
	{
