...
uint8_t RF_GetByte()
{
	...
	{
		_delay_us(750);
		th=rfi_i & rfi_in&0x01;  
		data|=(th<<i);  // nhan bit thap
		if(rfi_i & rfi_in)while(rfi_i & rfi_in);
		else while((rfi_i & rfi_in)==0);
	}
	return data;
}          
//======================================== 
uint8_t RF_GetDataStart()
{
    ...
    else                                 // kiem tra canh xuong
	{
        ...
        if((rfi_i & rfi_in)==0)
        {
            while((rfi_i & rfi_in)==0);       // cho cho den khi het bit 0
            while((rfi_i & rfi_in)==1);        // cho cho den khi het bit 1 tiep theo
            for(i=0;i<22;i++)         // neu dung 20 bit 1 thi dung là 3 byte 0xff
            {          
                _delay_us(750);
                if(rfi_i & rfi_in)
...
//======================================== 
u8 RF_GetPulseFinish()
{
    uint16_t i;
    while((rfi_i & rfi_in)==0);
    while(rfi_i & rfi_in);
    for(i=0;;i++)
    {
        _delay_us(200);
