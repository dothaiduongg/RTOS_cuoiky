...
/*  ************************************************************ */
/*  Mikrocontroller:  ATMEL AVR ATmega32, 12 MHz                  */
/*                                                               */
...
void LCD_Write_Byte_4_bit(u8 command)
{
	...
	delay_us(dlglcd); //(>4)					// Please increase this delay if the screen is not showing things correctly.
}
...
//{
	...
	////refresh_lcd = 1;
//}
void glcd_pixel_mang(int8 x, int8 y, u8 mau)// khong dung ram vi dieu khien
{
	int8 o_16bit, dong_32, b;
	dong_32 = y;
	o_16bit = x/16;
...
void glcd_mang_ghi(u8 x, u8 y)//, u8 color)// khong dung ram vi dieu khien
{
	int8 o_16bit, dong_32;
	dong_32 = y;
	o_16bit=x/16;
	if (o_16bit_cu!=o_16bit||dong_32_cu!=dong_32)
	{
...
void glcd_doc_mang_du_lieu(int8 x, int8 y)
{
	...
	if (o_16bit_cu!=o_16bit||dong_32_cu!=dong_32)
	{
		...
		v1 = lcd_read_4_bit(1);
		v2 = lcd_read_4_bit(1);
		if (v1!=v1_nho||v2!=v2_nho)
		{
			v1_nho=v1;
			v2_nho=v2;
			lcd_khong_tot=0;
		}
		glcd_mang_dl[glcd_vi_tri_mang]=v1;
		glcd_mang_dl[glcd_vi_tri_mang]<<=8;
		glcd_mang_dl[glcd_vi_tri_mang]|=v2;
		o_16bit_cu=o_16bit;
		dong_32_cu=dong_32;
		glcd_vi_tri_mang++;
	}
...
void glcd_ghi_ky_tu_5x7(u8 x, u8 y,u8 ki_tu, u8 size)
{
	...
	//for(; *textptr != '\0'; ++textptr, ++x)// Loop through the passed string
	{
		...
		for(i = 0; i < do_rong_ky_tu; i++)
		{
			pixelData[i]=(pgm_read_byte(&font5x7_hoa[((ki_tu-0x20) * 5) + i]));
			lx=lx+size;
		}
		
		for(j=0; j<do_rong_ky_tu; ++j, x+=size)         // Loop through character byte data
		{
			glcd_vi_tri_mang=0;
			lcd_khong_tot=1;
			for(k=0; k < 7; ++k)             // Loop through the vertical pixels
			...
			for(k=0; k < 7; ++k)             // Loop through the vertical pixels
			{
				if(pixelData[j] & (1<<k)) // Check if the pixel should be set
				{
					...
					{                          // character's size
						for(m=0; m < size; ++m)
						{
							glcd_pixel_mang(x+m, y+k*size+l, 1); // Draws the pixel
						}
					}
				}
				else
				{
					...
					{                          // character's size
						for(m=0; m < size; ++m)
						{
							glcd_pixel_mang(x+m, y+k*size+l, 0); // Draws the pixel
						}
					}
				}
			}
...
void glcd_ghi_chuoi_57(u8 x, u8 y, char* textptr, u8 size)
{
	//if (che_do_graphic!=1)
	//{
		//LCD_EnableGraphics(); // neu muon ghi chu tu font ngoai hoac hinh anh phai chuyen qua che do Graphic
...
void glcd_ki_tu57_gram_ram_re(u8 x, u8 y,u8 ki_tu, u8 size, u8 color)
{
	u8 x1,x2,x3;
	if (che_do_graphic!=1)
	{
...
void glcd_ki_tu57_gram_ram(u8 x, u8 y,u8 ki_tu, u8 size, u8 color)
{
	u8 x1,x2,x3;
	u8 do_rong_ky_tu=5;
	if (che_do_graphic!=1)
...
//========================================
u8 tap_ma_khoi_tao_lcd[6]={0b00100000,0b00100000,0b00001100,0x01,0x06,0b00000010};
void LCD_Init_4bit(void)
{
	...
	for (i=0;i<6;i++)
	{
		LCD_WriteCommand(tap_ma_khoi_tao_lcd[i]); // function set, 4 bit mode
		delay_ms(10);
	}
