...
void enable_data(void) 		{ SHT_DDR|=_BV(SHT_DATA);}			// Drive DATA pin
void disable_data(void)		{ SHT_DDR&=~(_BV(SHT_DATA)); 		// Release DATA pin
SHT_PORT|=_BV(SHT_DATA); }
void data_high(void)		{ SHT_PORT|=_BV(SHT_DATA); }		// DATA pin high
void data_low(void)			{ SHT_PORT&=~(_BV(SHT_DATA)); }		// DATA pin low
void clock_high(void)		{ SHT_PORT|=_BV(SHT_CLOCK); } 		// CLOCK pin high
void clock_low(void)		{ SHT_PORT&=~(_BV(SHT_CLOCK)); }	// CLOCK pin low
...
void ShtInit(void)
{
	SHT_DDR |= _BV(SHT_CLOCK);		// Set clock pin to output
	SHT_PORT &= ~ _BV(SHT_CLOCK);		// Turn off clock pin
	disable_data();
}
...
void transstart(void)
{
	...
	clock_high();	_delay_us(5);
	data_low();		_delay_us(5);
	clock_low();	_delay_us(10);
	clock_high();	_delay_us(5);
	data_high();	_delay_us(5);
	clock_low();
}
...
char ShtReadStatus(unsigned char *p_value)
{
	...
	*p_value = read_byte_SHT(); 			//read status register (8-bit)
