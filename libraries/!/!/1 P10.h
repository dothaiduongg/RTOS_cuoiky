...
uint8_t GFXWriteStringXY_s(uint8_t x,uint8_t y,const char *string,uint8_t color);
uint8_t GFXWriteStringXY_d(uint8_t x,uint8_t y,const char *string,uint8_t color);
int8_t GFXPutCharXY_d(int8_t x, int8_t y,char c,uint8_t mau);
uint8_t GFXGetCharWidth(char c);
uint8_t GFXWriteStringXY(uint8_t x,uint8_t y,const char *string,uint8_t color);
...
int8_t GFXPutCharXY_l(int8_t x, int8_t y,char c,uint8_t mau) // mau = 1 hien thi, mau = 2 soa diem, 3 soa vug
{
...
//========================================
uint8_t GFXGetStringWidth(const char *string)
{
	uint8_t w,r;
	w=0;
	while(*string!='\0')
...
//========================================
uint8_t GFXGetStringWidthN(const char *string,uint8_t n)
{
	uint8_t w,r;
	uint8_t i=0;
	w=0;
...
//#define c1_8
void p10_hc595_2_mau3264(u8 data_d1,u8 data_d2,u8 data_d21,u8 data_d22, u8 data_s1,u8 data_s2,u8 data_s21,u8 data_s22)
{
	//Send each 8 bits serially
...
void quet_p10_2_mau3264(uint8_t lap)
{
	uint8_t j;
	...
	for (j=0;j<lap;j++)
	{
		for(uint8_t i=0;i<b_rog;i++)
		{
			...
			p10_hc595_2_mau3264(
			p10_vram_d[(b_rog-1-i)*cao+kenh_chon],p10_vram_d[(b_rog-1-i)*cao+kenh_chon+16],p10_vram_d[(b_rog-1-i)*cao+kenh_chon+16],p10_vram_d[(b_rog-1-i)*cao+kenh_chon+8+16],
			p10_vram_s[(b_rog-1-i)*cao+kenh_chon],p10_vram_s[(b_rog-1-i)*cao+kenh_chon+16],p10_vram_s[(b_rog-1-i)*cao+kenh_chon+16],p10_vram_s[(b_rog-1-i)*cao+kenh_chon+8+16]);
			//p10_hc595_2b(p10_vram[(7-i)*2+ch],p10_vram[(7-i)*4+ch]);
		}
		p10_of();
	}
...
void p10_hc595_2_mau16X32_2x2(u8 data_d1,u8 data_d2, u8 data_s1,u8 data_s2)
{
	//Send each 8 bits serially
...
void p10_hc595_2_mau16X32_2x2_nguoc_ve(u8 data_d1,u8 data_d2, u8 data_s1,u8 data_s2)
{
	//Send each 8 bits serially
	...
	for(uint8_t i=0;i<8;i++)
	{
		...
		//d22=0;
		//s22=0;
		if(!(data_d1 & 0b10000000))
		{
			d1=1;
			//s1=1;
		}
		data_d1=data_d1<<1;  //Now bring next bit at MSB positio
		if(!(data_d2 & 0b10000000))
		{
			d2=1;
			//s2=1;
		}
		...
		//data_d22=data_d22>>1;  //Now bring next bit at MSB position
		//-----------------------------------------------
		if(!(data_s1 & 0b10000000))
		{
			l1=1;
		}
		data_s1=data_s1<<1;  //Now bring next bit at MSB positio
		if(!(data_s2 & 0b10000000))
		{
			l2=1;
		}
		data_s2=data_s2<<1;
...
//======================================== 
void quet_p10_2_mau16X32_2x2(uint8_t lap)
{
	uint8_t j;
	p10_of();
	for (j=0;j<lap;j++)
	{
		for(uint8_t i=0;i<b_rog;i++)
		{
			...
			p10_hc595_2_mau16X32_2x2_nguoc_ve(
			//p10_vram_d[(b_rog-1-i)*cao+kenh_chon],p10_vram_d[(b_rog-1-i)*cao+kenh_chon+16],p10_vram_d[(b_rog-1-i)*cao+kenh_chon+16],p10_vram_d[(b_rog-1-i)*cao+kenh_chon+8+16],
			//p10_vram_s[(b_rog-1-i)*cao+kenh_chon],p10_vram_s[(b_rog-1-i)*cao+kenh_chon+16],p10_vram_s[(b_rog-1-i)*cao+kenh_chon+16],p10_vram_s[(b_rog-1-i)*cao+kenh_chon+8+16]);
			p10_vram_d[(i)*cao+31-kenh_chon],p10_vram_d[(i)*cao+23-kenh_chon], // lấy hàng đỏ
			p10_vram_s[(i)*cao+31-kenh_chon],p10_vram_s[(i)*cao+23-kenh_chon]); // lấy hàng xanh
			
			
			//p10_hc595_2_mau16X32_2x2(
			//p10_vram_d[(b_rog-1-i)*cao+kenh_chon],p10_vram_d[(b_rog-1-i)*cao+kenh_chon+8],
			//p10_vram_s[(b_rog-1-i)*cao+kenh_chon],p10_vram_s[(b_rog-1-i)*cao+kenh_chon+8]);
			//p10_hc595_2b(p10_vram[(7-i)*2+ch],p10_vram[(7-i)*4+ch]);
		}
		for(uint8_t i=0;i<b_rog;i++)
		{
...
//======================================== 
void p10_hc595_2_mau16X32_2x2_4s(u8 data_d1,u8 data_d2, u8 data_s1,u8 data_s2)
{
	//Send each 8 bits serially
...
void p10_hc595_2_mau16X32_2x2_4s_nguoc_ve(u8 data_d1,u8 data_d2, u8 data_s1,u8 data_s2)
{
	//Send each 8 bits serially
...
//========================================
void quet_p10_2_mau16X32_2x2_4s(uint8_t lap) // lai quet 1/4
{
	uint8_t j;
	p10_of();
	for (j=0;j<lap;j++)
	{
		for(uint8_t i=0;i<b_rog;i++)
		{
			p10_hc595_2_mau16X32_2x2_4s_nguoc_ve
			(
				p10_vram_d[(i)*cao+31-kenh_chon],p10_vram_d[(i)*cao+23-kenh_chon], // lấy hàng đỏ
				p10_vram_s[(i)*cao+31-kenh_chon],p10_vram_s[(i)*cao+23-kenh_chon] // lấy hàng xanh
			); 
			p10_hc595_2_mau16X32_2x2_4s_nguoc_ve
			(
				p10_vram_d[(i)*cao+27-kenh_chon],p10_vram_d[(i)*cao+19-kenh_chon], // lấy hàng đỏ
				p10_vram_s[(i)*cao+27-kenh_chon],p10_vram_s[(i)*cao+19-kenh_chon] // lấy hàng xanh
			); 
		}
		for(uint8_t i=0;i<b_rog;i++) // b_rog=8
		{
			p10_hc595_2_mau16X32_2x2_4s // lấy dữ liêu 4 hàng dưới
			(
				p10_vram_d[(b_rog-1-i)*cao+kenh_chon],p10_vram_d[(b_rog-1-i)*cao+kenh_chon+8],
				p10_vram_s[(b_rog-1-i)*cao+kenh_chon],p10_vram_s[(b_rog-1-i)*cao+kenh_chon+8]
			);
			p10_hc595_2_mau16X32_2x2_4s // lấy dữ liệu 4 hàng trên
			(
				p10_vram_d[(b_rog-1-i)*cao+kenh_chon+4],p10_vram_d[(b_rog-1-i)*cao+kenh_chon+12],
				p10_vram_s[(b_rog-1-i)*cao+kenh_chon+4],p10_vram_s[(b_rog-1-i)*cao+kenh_chon+12]
			);
		}
	}
	/*
	if(kenh_chon==0) // 
	{									 	// ten ben trai				
		p10_hc595_2_mau16X32_2x2_loai_2(0b01111111,255,255,255);
											// xuong 4
		p10_hc595_2_mau16X32_2x2_loai_2(0b10111111,255,255,255);
		p10_hc595_2_mau16X32_2x2_loai_2(0b11011111,255,255,255);
		p10_hc595_2_mau16X32_2x2_loai_2(0b11101111,255,255,255);
		p10_hc595_2_mau16X32_2x2_loai_2(0b11110111,255,255,255); 
		p10_hc595_2_mau16X32_2x2_loai_2(0b11111011,255,255,255); 
		p10_hc595_2_mau16X32_2x2_loai_2(0b11111101,255,255,255); 
		p10_hc595_2_mau16X32_2x2_loai_2(0b11111110,255,255,255); 
	}
	if(kenh_chon==1)
	{
		p10_hc595_2_mau16X32_2x2_loai_2(255,255,255,255);
		p10_hc595_2_mau16X32_2x2_loai_2(255,255,255,255);
		p10_hc595_2_mau16X32_2x2_loai_2(255,255,255,255);
		p10_hc595_2_mau16X32_2x2_loai_2(255,255,255,255);
		p10_hc595_2_mau16X32_2x2_loai_2(255,255,255,255);
	...
	p10_on();
	kenh_chon++;
	if(kenh_chon>4-1)//kieu_quet-1)
	kenh_chon=0;
}



void p10_hc595_1_mau16X32_2x2x2(u8 data_d1,u8 data_d2)
{
	//Send each 8 bits serially
...
void p10_hc595_1_mau16X32_2x2x2_nguoc_ve(u8 data_d1,u8 data_d2)
{
	//Send each 8 bits serially
...
//========================================
//u8 lan=0;
void quet_p10_1_mau16X32_x2x4(uint8_t lap) // 4 tam P4 64x64
{
	uint8_t j;
	p10_of();
	for (j=0;j<lap;j++)
...
// 			//p10_hc595_2bx2(p10_vram[(so_byte_rog-1-i)*cao+ch],p10_vram[(so_byte_rog-1-i)*cao+ch+8],p10_vram[(so_byte_rog-1-i)*cao+ch+16],p10_vram[(so_byte_rog-1-i)*cao+ch+8+16],0);
// 			
// 			p10_hc595_1_mau16X32_2x2x2_nguoc_ve(
// 			//p10_vram_d[(b_rog-1-i)*cao+kenh_chon],p10_vram_d[(b_rog-1-i)*cao+kenh_chon+16],p10_vram_d[(b_rog-1-i)*cao+kenh_chon+16],p10_vram_d[(b_rog-1-i)*cao+kenh_chon+8+16],
// 			//p10_vram_s[(b_rog-1-i)*cao+kenh_chon],p10_vram_s[(b_rog-1-i)*cao+kenh_chon+16],p10_vram_s[(b_rog-1-i)*cao+kenh_chon+16],p10_vram_s[(b_rog-1-i)*cao+kenh_chon+8+16]);
// 			p10_vram_d[(i)*cao+31-kenh_chon/2],p10_vram_d[(i)*cao+23-kenh_chon/2]);//, // lấy hàng đỏ
// 			//p10_vram_s[(i)*cao+31-kenh_chon],p10_vram_s[(i)*cao+23-kenh_chon]); // lấy hàng xanh
// 			
// 			
// 			//p10_hc595_2_mau16X32_2x2(
...
// 		}
		for(uint8_t i=0;i<b_rog;i++)
		{
			...
			{
				p10_hc595_1_mau16X32_2x2x2_nguoc_ve(p10_vram_d[(i-5)*cao+31-kenh_chon/2],p10_vram_d[(i-5)*cao+23-kenh_chon/2]);
				//p10_hc595_1_mau16X32_2x2x2_nguoc_ve(p10_vram_d[(b_rog-1-i)*cao+kenh_chon/2],p10_vram_d[(b_rog-1-i)*cao+kenh_chon/2+8]);//)//,
			}
			else
			p10_hc595_1_mau16X32_2x2x2(p10_vram_d[(b_rog-1-i)*cao+kenh_chon/2],p10_vram_d[(b_rog-1-i)*cao+kenh_chon/2+8]);//)//,
		}
		//p10_of();
	}
