...
void showDigit(char ch, int col, const uint8_t *data)
{
  ...
  for (int i = 0; i < w; i++)
    if(col+i>=0 && col+i<NUM_MAX*8) {
      ...
      if(!dy) scr[col + i].byte_data = v; else scr[col + i].byte_data |= dy>0 ? v>>dy : v<<-dy;
    }
}
...
int printChar(unsigned char ch, const uint8_t *data)
{
  ...
  int i,w = pgm_read_byte(data + 1 + ch * len);
  for (i = 0; i < w; i++)
    scr[NUM_MAX*8 + i].byte_data = pgm_read_byte(data + 1 + ch * len + 1 + i);
  scr[NUM_MAX*8 + i].byte_data = 0;
  return w;
}
